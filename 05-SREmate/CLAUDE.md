...
当涉及 UI 代码修改时（如 `src/main/resources/static/` 下的 HTML、CSS、JavaScript 文件），**必须使用 Playwright 验证 UI 正确性**。

### Playwright 安装状态

Playwright CLI 已全局安装（版本 1.58.2），无需额外配置。
...
# 截图（指定视口尺寸）
playwright screenshot http://localhost:8089/ontology.html screenshot.png --viewport-size=1920,1080

# 生成 PDF
playwright pdf http://localhost:8089/ontology.html output.pdf
...
1. **启动应用**：确保 Spring Boot 应用运行在正确端口
2. **截图验证**：使用 Playwright 截取页面截图
3. **对比检查**：确认 UI 显示符合预期（布局、数据、交互元素）
4. **保存截图**：重要变更的截图可提交到 git 作为文档
