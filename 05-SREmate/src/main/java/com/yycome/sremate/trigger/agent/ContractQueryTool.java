...
//
//    /**
//     * 根据项目订单号查询所有合同完整详情（含关联表，已废弃）
//     * @deprecated 推荐使用 queryContractListByOrderId，关联数据由 Agent 按需调用专用工具获取
//     */
//    @Tool(...)
//    public String queryContractsByOrderId(String projectOrderId) { ... }

    /**
     * 根据项目订单号查询合同列表（仅 contract 表数据）
     * 关联数据（节点、签约单据、扩展字段）由 Agent 按需调用专用工具获取：
     * queryContractNodes / queryContractSignedObjects / queryContractFields
     */
    @Tool(description = """
            【订单号查询合同列表】用户输入纯数字订单号时使用，返回该订单下所有合同的基础信息。
            调用此工具后立即停止，不要再调用其他任何工具。

            触发条件：编号为纯数字（如826031111000002817）

            返回内容：contractCode、type、status、amount、platformInstanceId、ctime

            参数：
            - projectOrderId：纯数字订单号（必填）

            注意：C前缀合同号请用 queryContractBasic 等专用工具""")
    @DataQueryTool
    public String queryContractListByOrderId(String projectOrderId) {
        return ToolExecutionTemplate.execute("queryContractListByOrderId", () -> {
            List<Map<String, Object>> result = contractDao.fetchContractsByOrderId(projectOrderId);
