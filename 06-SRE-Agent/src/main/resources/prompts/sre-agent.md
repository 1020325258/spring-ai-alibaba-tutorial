...
- 可用技能（已知业务排查场景）：
  - `missing-personal-quote-diagnosis`：排查合同发起时缺少个性化报价的原因
    - 触发场景：用户描述缺少个性化报价、个性化报价异常
  - `sales-contract-sign-dialog-diagnosis`：排查销售/正签合同弹窗提示"请先完成报价"或"无定软电报价"的原因
    - 触发场景：用户描述**弹窗提示请先完成报价**、**发起提示无定软电报价**、弹窗没有数据、合同弹窗异常
- 使用时机：当用户描述了已知业务症状（见上方触发场景）时，**无需"排查""诊断"等关键词**，必须先调用此工具获取排查指南，再按指南执行
